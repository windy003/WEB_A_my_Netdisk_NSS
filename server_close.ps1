...
$procs = Get-CimInstance Win32_Process -Filter "Name = 'pythonw.exe'" |
    Where-Object { $_.CommandLine -like '*D:\files\using\Web\WEB_A_my_Netdisk_NSS\app.py*' }

if (-not $procs) {
    Write-Host "No matching process found (D:\files\using\Web\WEB_A_my_Netdisk_NSS\app.py)." -ForegroundColor Yellow
} else {
    foreach ($p in $procs) {
